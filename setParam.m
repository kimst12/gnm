...
%model. The parameters that change can be found in lines 49-64; you can
% comment and uncomment the various sections to set the parameters you'd like to change.
...
%     param.l_cap=50*1e-6; % from Blinder et al, Nature Neuroscienc 2013 (pg 890)

% Set number of capillaries branching off of each penetrating artery
param.ncap_per_penart=11;

% Set pressure gradient generated by each type of vessel
% (The first 3 are pressure gradients, the 4th is a pressure difference.)
param.dpdx=[0 0 0 0]; % 40 mmHg / 0.2 m = 200 is max from pulsations

% Short-circuit network? 1 - yes, 0 - no
param.sc=0;
...
%    param.kappa=[nan 1.8e-14 1.8e-14]; % porous
%    param.kappa=[nan nan 1.8e-14]; % open

printR=1; %choose whether or not to display the calculated Resistance values
%% -----------------------------------------------------------------------
%% Calculations derrived from parameters that change
%---------------------------------------------------------------------
% Calculate R_par: Resistance to flow from the pen
% nodes to venous side, through the parenchyma. mmHg-min/mL
...
% 1D:
%     A=param.l_penart/param.ncap_per_penart*L_art_to_art; % units m^2
%     R_par=L_art_to_ven/(Lp*A*60*1e2);
%     disp(['R_par = ' num2str(R_par,3)])
% 2D:
Lp=Lp/(133*100^2);
R_par=log((1-2*L_art_to_ven/param.d_penart)^2)/(2*pi*Lp*param.l_penart/param.ncap_per_penart); % from S8 in % https://www.pnas.org/content/pnas/suppl/2017/08/23/1706942114.DCSupplemental/pnas.201706942SI.pdf?targetid=nameddest%3DSTXT
R_par=R_par*cf;
...
R_efg=12*mu*T/(g^2*ef_cf*2*pi*(param.d_penart/2)*sqrt(1+param.K(2))*(param.l_penart/param.ncap_per_penart)); % JT: I added the sqrt(1+param.K(2)) term on 5/28/21
R_efg=R_efg*cf;
...
% perivascular space
% Units: mL/(min*mmHg)
R_vas=0.4; % poise/micron^3 this comes from Blinder 2013
mu_h2o=0.0091; %poise, viscocity for water, what Blinder used
r_vas=2; % microns
mult=4.*(1 - 0.863*exp(-r_vas/14.3) + 27.5*exp(-r_vas/0.351)); % multiplier to correct for the granular nature of blood; in the online methods section of Blinder 2013
...
param.l_cap=L_vas*1e-6; %This way you don't have to make any changes to the main model
